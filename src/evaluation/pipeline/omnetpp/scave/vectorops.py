...
def _combine_rows(df, out_times, out_values, comment):
    def column_brief(df, col):
        ...
        if n == 0:
            ...
            for i in ["", np.nan,
                      None]:  # move empty values to the back so that aren't chosen as example if at all possible
                if i in names:
                    names.remove(i);
                    names.append(i)  # move to back
...
def _integrate_helper(t, v, interpolation):
    ...
    if interpolation == 'sample-hold':
        ...
        raise ValueError(
            "unknown interpolation '{}', available ones are: 'linear', 'sample-hold', 'backward-sample-hold'",
