...
class ChartTemplate:
    ...
    def __init__(self, id: str, name: str, type: str, icon: str, script: str, dialog_pages, properties):
        ...
        - `pos` (string): A short string that uniquely identifies the chart template.
    ...
    def create_chart(self, id: int = None, name: str = None, props=None):
        ...
        - `pos` (string): A numeric string that identifies the chart within the
        ...
        if props:
            ...
            if unknown_keys:
                raise KeyError(
                    "Refusing to set chart properties not defined in the chart template {}: {}".format(repr(self.id),
                                                                                                       str(unknown_keys)))
            all_props.update(props)
        return analysis.Chart(type=self.type, name=name, template=self.id, icon=self.icon, script=self.script,
                              dialog_pages=self.dialog_pages.copy(), properties=all_props)
...
def load_chart_template(properties_file):
    ...
    i = 0
    while True:
        page_id = get(p, "dialogPage." + str(i) + ".pos")
        if page_id is None:
            break
    ...
    return ChartTemplate(id=id, name=name, type=type, icon=icon, script=script, dialog_pages=dialog_pages,
...
def _collect_editable_properties(dialog_pages):
    ...
    for page in dialog_pages:
        ...
        for e in root.iter():
            id = e.attrib.get('{http://sweet_swt.sf.net/xswt}pos')
            if id is not None:
                default = e.attrib.get('{http://sweet_swt.sf.net/xswt}pos.default')
