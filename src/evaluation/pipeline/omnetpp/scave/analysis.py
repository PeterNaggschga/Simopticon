...
def _make_id(id_attr: str):
    ...
    try:
        ...
        raise RuntimeError(f"Wrong Chart or Folder pos '{id_attr}': IDs are expected to be numeric strings")
...
class DialogPage:
    ...
    def __init__(self, pos: str = None, label: str = "", content: str = ""):
        self.id = id
        self.label = label
...
class Chart:
    ...
    def __init__(self, pos: str = None, name: str = "", type: str = "MATPLOTLIB", template: str = None,
                 icon: str = None,
                 script: str = "", dialog_pages=list(), properties=dict()):
    ...
    def __repr__(self):
        return f"Chart(type='{self.type}',name='{self.name}',pos={self.id})"
...
class Folder:
    ...
    def __init__(self, pos: str = None, name: str = "", items=list()):
        self.id = _make_id(id)
        self.name = name
        self.items = items
...
class Workspace:
    ...
    def __init__(self, workspace_dir=None, project_locations=None):
        ...
        if workspace_dir:
            if not os.path.isdir(workspace_dir):
                raise RuntimeError(
                    "Specified workspace directory doesn't exist, or is not a directory: " + workspace_dir)
    ...
    def get_project_name(self, project_dir):
        ...
        try:
            ...
            names = [name.firstChild.nodeValue for name in project_dom.getElementsByTagName("name") if
                     name.parentNode.tagName == "projectDescription"]
    ...
    def get_project_location(self, project_name):
        ...
        if self.workspace_dir and os.path.isdir(self.workspace_dir + "/.metadata"):
            ...
            if not os.path.isfile(location + "/.project"):
                raise RuntimeError(
                    f"Cannot determine location for project '{project_name}' from Eclipse workspace data: stated location '{location}' is not valid")
        else:
            ...
            if location is None or not os.path.isfile(location + "/.project"):
                raise RuntimeError(
                    f"Cannot determine location for project '{project_name}': It is not next to any known project, and an Eclipse workspace is not available for more info")
    ...
    def to_filesystem_path(self, wspath):
        ...
        if wspath.startswith('/'):
            ...
            if not os.path.isdir(project_loc):
                raise RuntimeError(
                    f"Directory for project {project_name} doesn't exist (workspace dir not specified?): {project_loc}")
...
class Analysis:
    ...
    def from_anf_file(anf_file_name):
        ...
        def make_chart(chart_elem):
            ...
            dialog_pages = [DialogPage(id=dp.get('pos'), label=dp.get('label'), content=content(dp)) for dp in
                            chart_elem.findall('dialogPage')]
            props = {p.get('name'): p.get('value') for p in chart_elem.findall('property')}

            return Chart(
                id=chart_elem.get('pos'),
    ...
    def export_image(self, chart, wd, workspace, format="svg", target_folder=None, filename=None, width=None,
                     height=None, dpi=96, enforce=True, extra_props=dict()):
        """
    ...
    def export_data(self, chart, wd, workspace, format="csv", target_folder=None, filename=None, enforce=True,
                    extra_props=dict()):
        """
    ...
    def to_anf_file(self, filename):
        ...
        def makeFolderEl(folder, tag='folder'):
            ...
            setAttr(folderEl, 'pos', folder.id)
        ...
        def makeChartEl(chart):
            ...
            for dialog_page in chart.dialog_pages:
                ...
                setAttr(dialogpageEl, "pos", dialog_page.id)
