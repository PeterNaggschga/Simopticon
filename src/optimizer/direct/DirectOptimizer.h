...
private:
     ...
     * They are grouped by HyRect::t and sorted by HyRect::avgValue which simplifies the search for potentially optimal rectangles in DirectOptimizer::optimalRectangles.
     */
     ...
     * in the parameter space using DirectOptimizer::normalizer and calls Optimizer::requestValues.
     ...
     * First filters for only the best rectangles of a size from a subset of all DirectOptimizer::activeRects determined by DirectOptimizer::level.
     ...
     * @param nrRects: Size of the partition (meaning number of rectangles in DirectOptimizer::activeRects).
    ...
    /**
     * Requests values at the corners of the given rectangles and add all given HyRect instances to DirectOptimizer::activeRects.
     * @param rects: Rectangles to be evaluated and added.
     */
    void addActiveRects(const list<shared_ptr<HyRect>> &rects);

    /**
     * Removes the given rectangles from DirectOptimizer::activeRects.
     ...
     * @param nrRects: Number of rectangles in the current partition (meaning number of rectangles in DirectOptimizer::activeRects).
     */
...
public:
     ...
     * Only returns when an iteration has completed and DirectOptimizer::stopCon deems the optimization complete or when Optimizer::abort was called in the last iteration.
     */
    ...
    /**
     * Returns the number of rectangles stored in DirectOptimizer::activeRects.
