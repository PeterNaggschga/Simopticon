...
private:
    /**
     * Number of repetitions per Parameter combination and scenario in PlexeSimulationRunner::SCENARIOS.
    ...
    size_t runNumber = 0;
    /**
     * Threadlock to prevent race conditions on concurrent access of PlexeSimulationRunner::runNumber.
     */
    mutex runNumberLock;
     ...
     * Basically increments PlexeSimulationRunner::runNumber and returns value before incrementation.
     ...
     * Parallelizes the execution of different scenarios (see PlexeSimulationRunner::SCENARIOS) and their repetitions (see PlexeSimulationRunner::REPEAT) using Multithreaded class.
     * Parallelized function is defined in PlexeSimulationRunner::work(std::pair< std::filesystem::path, std::pair< std::basic_string< char >, unsigned int >>).
...
public:
     ...
     * The new PlexeSimulationRunner uses @f$ t = \min(\mathit{threads}, \mathit{repeat} \cdot \mathit{size}(\mathit{scenarios})) @f$ concurrent threads for parallelization of PlexeSimulationRunner::work(std::pair< std::filesystem::path, std::pair< std::basic_string< char >, unsigned int >>).
     * For the parallelization of PlexeSimulationRunner::work(vector<shared_ptr<Parameter>>) @f$ t' = \lfloor \mathit{threads} \div t \rfloor @f$ concurrent threads are used.
